...
export default function CalendarPage() {
      ...
      for (const store of stores) {
        for (const wr of WEEK_ROLES) {
          const exists = projects.find(
            p => p.store_id === store.id && p.week_number === wr.week
          );
          if (!exists) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    const labels: Record<string, string> = {
      completed: '完了', in_progress: '制作中', review: 'レビュー', planning: '企画',
    };
    return (
      <span className={`text-xs px-2 py-0.5 rounded ${colors[status] || colors.planning}`}>
        {labels[status] || '未着手'}
      </span>
    );
  };
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              {stores.map(s => (
                <th key={s.id} className="px-4 py-3 text-left font-medium text-gray-600">
                  {s.name}
                </th>
              ))}
            </tr>
                      ...
                      <td key={store.id} className="px-4 py-4">
                        {proj ? (
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">{proj.theme}</p>
                          </div>
                        ) : (
