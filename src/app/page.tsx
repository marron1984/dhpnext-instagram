  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stores.map(store => {
          const storeProjects = projects.filter(p => p.store_id === store.id);
          const completed = storeProjects.filter(p => p.status === 'completed').length;
          return (
            <div key={store.id} className="bg-white rounded-lg shadow p-5 border border-gray-200">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">
                  進捗: {completed}/{storeProjects.length}
                </span>
                <div className="w-20 bg-gray-200 rounded-full h-2">
                  <div
                    className="bg-blue-600 h-2 rounded-full"
                    style={{ width: `${storeProjects.length > 0 ? (completed / storeProjects.length) * 100 : 0}%` }}
                  />
                </div>
              </div>
            </div>
      ...
      {/* Week Roles Reference */}
      <div className="bg-white rounded-lg shadow border border-gray-200 mb-8">
        <div className="p-5 border-b border-gray-200">
          <h2 className="font-bold text-lg">月内投稿設計</h2>
        </div>
      ...
      {/* Weekly Schedule & Current Projects */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow border border-gray-200">
          <div className="p-5 border-b border-gray-200">
            <h2 className="font-bold text-lg">週次スケジュール</h2>
          </div>
          ...
          <div className="p-5">
            {projects.length === 0 ? (
              <p className="text-gray-500 text-center py-4">
                プロジェクトがありません。
                <Link href="/calendar" className="text-blue-600 hover:underline ml-1">カレンダーから作成</Link>
              </p>
            ) : (
